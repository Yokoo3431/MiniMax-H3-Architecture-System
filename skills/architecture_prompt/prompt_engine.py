"""Main Architecture Prompt Skill Engine Entrance (V0.7.1.6 Upgraded).
Integrates IntentParser -> KnowledgeMapper -> ReasoningEngine -> PromptBuilder -> QualityEvaluator.
"""
...
class ArchitecturePromptEngine:
    ...
    def process_request(self, text: str) -> dict:
        ...
        extra_keywords = list(set(km_res["mapped_keywords"] + reasoning_res["reasoning_prompts"]))
        if extra_keywords:
            pos_prompt = f"{pos_prompt}, {', '.join(extra_keywords)}"

        # 5. Quality Evaluation (5 Dimensions)
